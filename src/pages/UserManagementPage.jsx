...
function QRPrintModal({ isOpen, user, onClose }) {
    ...
    useEffect(() => {
        if (isOpen && user) {
            qrcodeLib.toString(user.customId || user.dni, { type: 'svg', level: 'H', margin: 1 })
            .then(setQrSvg).catch(err => console.error(err));
        }
    }, [isOpen, user]);
    ...
    const handlePrint = () => {
        ...
        printWindow.document.write(`<html><head><style>${FOTOCHECK_STYLES}</style></head><body><div class="card"><div class="header"><div class="logo-txt">S.I. GONZALES</div><div class="subheader">IDENTIFICACIÓN</div></div><div class="content"><div class="name">${user.name}<br>${user.lastName}</div><div class="qr-container">${qrSvg}</div><div style="font-size: 10px; color: #666; margin-top: 5px;">DNI: ${user.dni}</div></div><div class="footer-role">${user.role || user.type}</div></div></body></html>`);
        ...
        <div style={st.backdrop} onClick={(e) => e.target === e.currentTarget && onClose()}>
            <div style={st.qrPreviewCard}>
                ...
                <div style={{padding: '20px', backgroundColor: 'white'}}>
                    <div dangerouslySetInnerHTML={{ __html: qrSvg }} style={{width: '180px', height: '180px', margin: '0 auto'}} />
...
const UserManagementPage = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        accessLevel: 'Usuario', type: 'Trabajador', weeklyBonus: 0, birthday: ''
    });
    ...
    useEffect(() => { if (tienePermisoEscritura) fetchUsers(); }, [fetchUsers, tienePermisoEscritura]);
    ...
    const toggleSelectAll = () => {
        if (selectedUserIds.length === filteredUsers.length && filteredUsers.length > 0) setSelectedUserIds([]);
    ...
    const handlePrintSelectedQR = async () => {
        ...
        if (usersToPrint.length === 0) return alert("Selecciona usuarios primero");
        const printWindow = window.open('', '_blank');
        ...
        for (const u of usersToPrint) {
            ...
            html += `<div class="card"><div class="header"><div class="logo-txt">S.I. GONZALES</div><div class="subheader">IDENTIFICACIÓN</div></div><div class="content"><div class="name">${u.name}<br>${u.lastName}</div><div class="qr-container">${svg}</div><div style="font-size: 9px; color: #666;">DNI: ${u.dni}</div></div><div class="footer-role">${u.role || u.type}</div></div>`;
        }
        html += `</div></body></html>`;
        printWindow.document.write(html);
        printWindow.document.close();
        printWindow.onload = () => { setTimeout(() => { printWindow.print(); printWindow.close(); }, 600); };
    };

    const handleDelete = async (id, name) => {
        if(window.confirm(`¿Eliminar a ${name}?`)) {
            try { await axios.delete(`${apiUrl}/api/users/${id}`); fetchUsers(); } 
...
const updateRate = async (userId, currentName) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const data = {
                ...
                weeklyBonus: parseFloat(formData.weeklyBonus) || 0
            };
            if (isEditMode) await axios.put(`${apiUrl}/api/users/${selectedId}`, data);
            else await axios.post(`${apiUrl}/api/users`, data);
    ...
    const handleEdit = (u) => {
        const custom = u.role && !ALLOWED_ROLES.includes(u.role);
        setFormData({ 
            ...
            weeklyBonus: u.weeklyBonus || 0,
            birthday: u.birthday ? u.birthday.split('T')[0] : ''
        });
        setSelectedId(u._id); 
        setIsWorker(u.type === 'Trabajador');
        setIsCustomRole(custom);
        setIsEditMode(true); 
        setIsRegisterModalOpen(true);
    };

    const closeModal = () => {
        setIsRegisterModalOpen(false); setIsEditMode(false); setIsCustomRole(false);
        setFormData({ name: '', lastName: '', dni: '', phone: '', mail: '', role: '', accessLevel: 'Usuario', type: 'Trabajador', weeklyBonus: 0, birthday: '' });
    };
        ...
        <main style={st.container}>
            <header style={st.header}>
                ...
                <div style={st.actions}>
                    ...
                    <button onClick={() => setIsRegisterModalOpen(true)} style={st.btnPrimary}>➕ Nuevo</button>
                    <button onClick={handlePrintSelectedQR} style={{...st.btnSecondary, backgroundColor: selectedUserIds.length > 0 ? '#00ffa3' : '#3b4a54', color: selectedUserIds.length > 0 ? '#0b141a' : 'white'}}>
                        🖨️ Imprimir ({selectedUserIds.length})
                    </button>
                </div>
            </header>
            ...
            <div style={st.tableWrapper}>
                <table style={st.table}>
                    <thead>
                        <tr>
                            <th style={{...st.th, width: '40px'}}><input type="checkbox" onChange={toggleSelectAll} checked={filteredUsers.length > 0 && selectedUserIds.length === filteredUsers.length}/></th>
                            <th style={{...st.th, width: '40px'}}>N°</th>
                            <th style={st.th}>Apellidos y Nombres</th>
                            <th style={st.th}>🎂 Cumpleaños</th>
                            <th style={st.th}>Sueldo/Hr</th> 
                            <th style={st.th}>Bono Sem.</th> 
                    ...
                    <tbody>
                        {filteredUsers.map((u, index) => (
                            <tr key={u._id} style={{...st.tr, backgroundColor: selectedUserIds.includes(u._id) ? 'rgba(0, 255, 163, 0.05)' : 'transparent'}}>
                                <td style={st.td}><input type="checkbox" checked={selectedUserIds.includes(u._id)} onChange={() => toggleSelectUser(u._id)}/></td>
                                <td style={{...st.td, color: '#8696a0'}}>{index + 1}</td>
                                <td style={st.td}>{u.lastName}, {u.name}</td>
                                <td style={st.td}>
                                    {u.birthday ? (
                                    // Usamos split('T')[0] para tomar solo la parte de la fecha y no la hora
    ...
    <div style={{fontSize: '11px', color: '#8696a0'}}>
        Horas.Exta: S/ {(u.hourlyRate * 1.25).toFixed(2)} 
                                ...
                                <td style={st.tdActions}>
                                    ...
                                    <button onClick={() => updateRate(u._id, u.name)} style={st.btnMoney}>💰</button>
                ...
                <div style={st.backdrop} onClick={(e) => e.target === e.currentTarget && closeModal()}>
                    <div style={st.modalForm}>
                        ...
                        <form onSubmit={handleSubmit} style={st.formGrid}>
                            ...
                            <div style={{...st.inputGroup, borderLeft: '3px solid #e91e63'}}>
                                <label style={st.miniLabel}>🎂 Fecha de Cumpleaños</label>
                                <input type="date" value={formData.birthday} onChange={e=>setFormData({...formData, birthday: e.target.value})} style={st.inputTransparent}/>
                            </div>

                            <div style={{...st.inputGroup, borderLeft: '3px solid #ffca28'}}>
                                <label style={st.miniLabel}>💰 Bono Semanal (Pasajes/Cena)</label>
                                <input type="number" step="0.01" value={formData.weeklyBonus} onChange={e=>setFormData({...formData, weeklyBonus: e.target.value})} style={st.inputTransparent}/>
                            </div>

                            <label style={{color: '#8696a0', fontSize:'13px', display: 'flex', alignItems: 'center', gap: '8px'}}>
                                <input type="checkbox" checked={isWorker} onChange={e=>setIsWorker(e.target.checked)}/> ¿Es trabajador de obra?
                            </label>

                            {isWorker && (
                                <div style={{display: 'flex', flexDirection: 'column', gap: '5px'}}>
                                    {!isCustomRole ? (
                                        <select style={st.input} value={formData.role} onChange={e => e.target.value === "CUSTOM" ? setIsCustomRole(true) : setFormData({...formData, role: e.target.value})}>
                                            <option value="">Seleccionar Rol...</option>
                                            {ALLOWED_ROLES.map(c => <option key={c} value={c}>{c}</option>)}
                                            <option value="CUSTOM" style={{color: '#00ffa3'}}>➕ Otro (Escribir...)</option>
                                        </select>
                                    ) : (
                                        <div style={{display: 'flex', gap: '5px'}}>
                                            <input autoFocus placeholder="Escribe el rol..." value={formData.role} onChange={e=>setFormData({...formData, role: e.target.value})} style={{...st.input, flex: 1}}/>
                                            <button type="button" onClick={() => {setIsCustomRole(false); setFormData({...formData, role: ''})}} style={st.btnSecondary}>✖</button>
                                        </div>
                                    )}
                                </div>
                            )}
...
const st = {
    ...
    btnSecondary: { backgroundColor: '#3b4a54', color: 'white', border: 'none', padding: '10px 15px', borderRadius: '8px', cursor: 'pointer' },
    ...
    inputGroup: { backgroundColor: '#2a3942', padding: '8px', borderRadius: '8px', marginBottom: '5px' },
    miniLabel: { fontSize: '10px', color: '#8696a0', display: 'block', marginBottom: '2px' },
    inputTransparent: { background: 'none', border: 'none', color: 'white', width: '100%', outline: 'none', fontSize: '14px' },
    input: { backgroundColor: '#2a3942', border: '1px solid #3b4a54', padding: '12px', borderRadius: '8px', color: 'white', width: '100%', boxSizing: 'border-box' },
    modalButtons: { display: 'flex', gap: '10px', marginTop: '20px', justifyContent: 'center' },
    qrPreviewCard: { backgroundColor: 'white', borderRadius: '15px', textAlign: 'center', overflow: 'hidden', width: '300px' },
